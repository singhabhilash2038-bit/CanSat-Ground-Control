...
async function _bootstrap() {
  UI.logConsole('Initializing system components...', 'info');
  ...
  try {
    ...
    UI.logConsole(`Map initialization failed: ${e.message}`, 'error');
  }
  ...
  try {
    ...
    UI.logConsole(`Attitude viewer initialization failed: ${e.message}`, 'error');
  }
  ...
  // 8. Inject Demo Data (so dashboard isn't completely empty)
  _injectDemoData();

  // 9. Force a layout recalculation for visualizers that might have initialized at 0 height
  setTimeout(() => {
    window.dispatchEvent(new Event('resize'));
  }, 500);

  UI.logConsole('System ready. Awaiting telemetry.', 'ok');
}
...
function _wireEventHandlers() {
  ...
  if (btnSimStart && btnSimStop && speedSelect) {
    btnSimStart.addEventListener('click', () => {
      if (AppState.isConnected) {
        UI.logConsole('Cannot start simulator while hardware is connected.', 'warn');
        return;
      }
      if (Simulator.isRunning()) return;

      UI.logConsole('Starting internal flight simulator...', 'info');
      const speed = parseFloat(speedSelect.value) || 1.0;

      // Start simulator
      Simulator.start({
        speedMultiplier: speed,
        onPacket: _onSerialLine,
        onPhaseChange: (from, to, alt) => {
          UI.logConsole(`[SIM] Phase transition: ${from} → ${to} (Alt: ${alt.toFixed(1)}m)`, 'info');
        },
        onComplete: () => {
          UI.logConsole('[SIM] Simulation completed (Recovery phase ended).', 'info');
          btnSimStop.click();
        }
      });
      ...
      // Force UI to show connected state for simulator
      const statusPill = document.getElementById('serial-status-pill');
      const statusLabel = document.getElementById('serial-status-label');
      if (statusPill && statusLabel) {
        statusPill.classList.add('connected');
        statusLabel.textContent = `SIMULATOR (${speed}×)`;
      }
    });
    ...
    btnSimStop.addEventListener('click', () => {
      ...
      Simulator.stop();
      UI.logConsole(`Simulator stopped. Packets emitted: ${Simulator.getPacketCount()}`, 'info');
...
═══════════════════════════════════════════════════════════ */
function _wireCommandButtons() {
  const btnSep = document.getElementById('btn-cmd-sep');
  const btnChute = document.getElementById('btn-cmd-chute');
  const btnRed = document.getElementById('btn-cmd-red');

  const sendSimCommand = (cmdString, label) => {
    if (!Simulator.isRunning()) {
      UI.logConsole(`[COMMAND] Cannot send ${label}: Simulator is not running.`, 'warn');
      return;
    }
    UI.logConsole(`[COMMAND] Transmitting to SIM: ${cmdString.trim()}`, 'info');
    Simulator.receiveCommand(cmdString);
  };

  if (btnSep) {
    btnSep.addEventListener('click', () => {
      sendSimCommand('CMD,SEP\n', 'MANUAL SEPARATION');
    });
  }

  if (btnChute) {
    btnChute.addEventListener('click', () => {
      sendSimCommand('CMD,CHUTE\n', 'EMERGENCY PARACHUTE');
    });
  }

  if (btnRed) {
    btnRed.addEventListener('click', () => {
      sendSimCommand('CMD,RED\n', 'REDUNDANT ACTIVATION');
    });
  }
}
